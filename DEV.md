...
| 路径 | 页面 | 说明 |
|:---|:---|:---|
| `/` | Home | 首页菜单墙，按日期倒序（唯一页面） |
| `/dish/:id` | （深链） | 直接打开对应菜品的详情弹窗（分享用） |

其余功能均为弹窗：右下角"上新"FAB → 新增/编辑弹窗；点卡片 → 详情弹窗。
...
| v1.2.0 | 2026.08.04 | systemd 守护（开机自启+崩溃自愈）、精简为"首页+两个弹窗"单页结构（删除搜索/统计页及旧页面文件）、DEV.md 全面更新 |
